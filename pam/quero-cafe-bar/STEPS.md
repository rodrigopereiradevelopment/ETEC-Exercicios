...
Abrir o Terminal (PowerShell ou CMD).
Acesse sua pasta de preferência:
cd Desktop
Clone o repositório:
git clone GITHUB_URL
cd quero-cafe-bar
Configure o Git Local:
...
cd backend
npm install
Configurar Variáveis de Ambiente: Crie o arquivo .env na pasta /backend baseando-se no conteúdo abaixo:
PORT=3000
...
Iniciar o MySQL: Abra o painel de controle do XAMPP e clique em Start no módulo MySQL.
Resolução de Problemas: Se o MySQL do XAMPP não iniciar (ficar vermelho ou fechar):
...
Criar o Schema: No MySQL Workbench ou phpMyAdmin, execute:
...
Build do projeto web: npm run build
Sincronizar: npx cap copy e npx cap open android """
